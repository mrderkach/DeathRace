...
namespace UnityStandardAssets.Vehicles.Car
{
    ...
    public class CarAIControl : MonoBehaviour
    {
        ...
        [SerializeField] [Range(0, 1)] private float m_CautiousSpeedFactor = 0.3f;               // percentage of max speed to use when being maximally cautious
        [SerializeField] [Range(0, 180)] private float m_CautiousMaxAngle = 70f;                  // angle of approaching corner to treat as warranting maximum caution
        [SerializeField] private float m_CautiousMaxDistance = 100f;                              // distance at which distance-based cautiousness begins
        [SerializeField] private float m_CautiousAngularVelocityFactor = 25f;                     // how cautious the AI should be when considering its own current angular velocity (i.e. easing off acceleration if spinning!)
        [SerializeField] private float m_SteerSensitivity = 0.35f;                                // how sensitively the AI uses steering input to turn to the desired direction
        [SerializeField] private float m_AccelSensitivity = 0.1f;                                // How sensitively the AI uses the accelerator to reach the current desired speed
        ...
        private void FixedUpdate()
        {
            if (m_Target == null || !m_Driving ||
                !gameObject.GetComponent<CarGeneral>()._manager.racing || 
                gameObject.GetComponent<CarGeneral>().finished)
            {
            ...
            else
            {
                ...
                Vector3 corrector = new Vector3();
                if (Time.time >= gameObject.GetComponent<CarGeneral>().BulletNextTime)
                {
                    if (CheckPlayer(gameObject.GetComponent<CarGeneral>()._manager.player, ref corrector))
                    {
                        gameObject.GetComponent<CarGeneral>().Fire(corrector);
                        gameObject.GetComponent<CarGeneral>().BulletNextTime = Time.time +
                            gameObject.GetComponent<CarGeneral>().bulletDelay;
                    }
                    else
                    {
                        for (int i = 0;
                            i < gameObject.GetComponent<CarGeneral>()._manager.enemies.transform.childCount;
                            ++i)
                        {
                            if (gameObject.GetComponent<CarGeneral>()._manager.enemies.transform.GetChild(i).gameObject != gameObject &&
                                CheckPlayer(gameObject.GetComponent<CarGeneral>()._manager.enemies.transform.GetChild(i).gameObject,
                                    ref corrector))
                            {
                                gameObject.GetComponent<CarGeneral>().Fire(corrector);
                                gameObject.GetComponent<CarGeneral>().BulletNextTime = Time.time +
        ...
        private bool CheckPlayer(GameObject player, ref Vector3 corrector)
        {
            Vector3 bulletSpawn = gameObject.GetComponent<CarGeneral>().bulletSpawn.transform.position;
            Vector3 fwd = gameObject.GetComponent<CarGeneral>().bulletSpawn.transform.forward;
            Vector3 dir = player.GetComponent<CarGeneral>().aim.position - bulletSpawn;
            if (((dir.magnitude < 250 && Vector3.Angle(fwd, dir) < 20) || 
                (dir.magnitude < 20 && Vector3.Angle(fwd, dir) < 80) ) && 
                Physics.Linecast(player.GetComponent<CarGeneral>().aim.position, bulletSpawn))
            {
                corrector = dir.normalized;
                return true;
            }
